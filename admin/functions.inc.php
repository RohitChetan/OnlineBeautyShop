...
function get_safe_value($con, $str){
    if (!empty($str)) {
        if (is_array($str)) {
            // If $str is an array, apply trim() to each element
            foreach ($str as $key => $value) {
                $str[$key] = trim($value);
                $str[$key] = mysqli_real_escape_string($con, $str[$key]);
            }
            // Return the sanitized array
            return $str;
        } else {
            // If $str is a string, apply trim() and escape it
            $str = trim($str);
            return mysqli_real_escape_string($con, $str);
        }
    }
